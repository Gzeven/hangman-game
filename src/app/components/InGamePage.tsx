...
const InGamePage: React.FC = () => {
  ...
  const getPopupContent = () => {
    if (isWordGuessed()) {
      return { text: "You Win", buttonText: "Play Again" };
    } else if (health === 0) {
      return { text: "You Lose", buttonText: "Play Again" };
    } else {
      return { text: "Paused", buttonText: "Continue" };
    }
  };
  ...
  }, [selectedLetters, health, word, wrongSteps]);
  ...
  const isWordGuessed = useCallback((): boolean => {
  ...
  }, [selectedLetters, health, initialRender, isWordGuessed]);
  ...
  const handleContinue = () => {
    if (!isWordGuessed() && health !== 0) {
      ...
      const fetchNewWord = () => {
        if (currentCategory) {
          const categoryWords = (data.categories as Categories)[
            currentCategory
          ];
          ...
          if (unplayedWords.length > 0) {
            const randomIndex = Math.floor(
              Math.random() * unplayedWords.length
            );
  ...
  const renderWord = () => {
    ...
    const renderWordElements = words.map((segment, index) => (
      <LetterBoxContent key={index}>
        {segment
          .split("")
          .map((char, charIndex) => renderCharacter(char, charIndex))}
